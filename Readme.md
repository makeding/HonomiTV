...
Jellyfin Live TV は既存の `/tv/`、`/timetable/`、`/tv/watch/:display_channel_id` で利用し、
共通チャンネル分類 `IPTV` を使い、タブ内の短い表示名だけ「ネット」とする。
説明文では「ネットテレビ」と表記する。専用ページやナビゲーションは追加しない。
チャンネル・ロゴは `/api/channels`、番組表は `/api/programs/timetable`、再生セッションは
`/api/streams/live` に統合し、供給元専用の公開 API は残さない。

- `jellyfin-<上流 ID>` を安定した識別子として使う。チャンネルと番組表は要求時に取得し、別の永続化モデルを作らない。
- チャンネル一覧と詳細は同じ EPG 選択処理を使う。現在番組は `start <= now < end`、NEXT は現在番組の終了以降（空白時間帯では現在時刻以降）に始まる最初の番組とする。終了していない番組を全チャンネル分まとめて取得し、チャンネル別・開始時刻順に選ぶ。
- 番組表は `end > query_start && start < query_end` の重なりを返し、両端をまたぐ番組も保持する。Jellyfin の時刻はオフセットを持つ実際の時点として扱い、オフセットなし・不正な時刻は記録して除外する。Web と Komorebi の表示時刻・日付・番組表の日境界は端末のローカル時刻を使い、チャンネルの国別補正は加えない。
- EPG 取得だけが失敗した場合はチャンネルと有効な上流の現在番組を保持し、一覧・詳細の `X-Channel-Source-Errors` と番組表の `source_errors` に理由を返す。NEXT が空という正常応答に偽装しない。
- `/api/channels` の本文は既存クライアントとの互換性のため、全てのトップレベル値をチャンネル配列に保つ。ネットテレビ取得の失敗理由は `X-Channel-Source-Errors` ヘッダーに ASCII エスケープ済み JSON で返す。
- 出所と利用可能な機能を公共型に明示する。放送専用 ID、未知の映像・音声情報、視聴者数は欠損を表す。
- IPTV のチャンネル見出しはホーム・視聴パネル・番組詳細でロゴと局名を使い、放送波向けのチャンネル番号や `Ch: --` を表示しない。局名との間隔は番号なしの配置に合わせ、番組更新時には変化させない。
- 既存のカード、番組詳細、ピン留め、切り替え、戻る操作を共有する。録画予約、データ放送は本統合の対象外で、利用不可を表示する。
- Jellyfin チャンネルの `remote_playback` は true とし、テレビ選択中はカード・番組表から既存の `OpenLive(display_channel_id)` を送る。認証・所有者確認・WebSocket 中継を共有し、供給元専用の分岐や認証情報の転送は追加しない。
- 遠隔送信の成功・失敗とも元のページ、スクロール位置、カードとフォーカスを保ち、ローカル再生へ移行しない。成功通知は送信受理だけを示す。オフライン時はテレビの接続確認と再試行を案内する。テレビ未選択時は既存のローカル再生を維持する。
- ネットワークテレビには読み込み・空・失敗・再試行の領域を確保する。再取得失敗で既存カードや現在番組、ピン留めを消さず、ローカル放送へのアクセスを維持する。
- 再生は共通セッションで HLS / MPEG-TS を選ぶ。放送専用イベント購読とデータ放送管理は起動しない。
- 再生エラーは DPlayer の既存通知と再起動ボタンを使い、別のエラーパネルを重ねない。
- IPTV のロゴは既存の固定枠内に中灰色の背景で全体を表示し、白い文字を含む透過ロゴのコントラストを確保する。ネット視聴では実況の取得・接続とコメント操作を無効にし、画質は `Direct` と表示する。
- ネット・BS4K を含むライブ初期化・切り替えでは、既存の背景画像生成と表示設定を維持し、SSE の到着前から読み込み背景を表示する。再生開始時に既存のフェードアウトを使う。
- Komorebi も既存のチャンネル・番組表・プレイヤーへ同じ共通 API を接続する。タブ名は「ネット」とし、リモコンの選択・切り替え・戻る操作とフォーカスを維持する。
- 切り替え、退出、初期化失敗、古い非同期応答でセッションを閉じる。サーバーも接続が途絶えたセッションを回収し、上流の終了失敗を再試行する。
- HLS の子プレイリスト・分割メディア・鍵を各プレイリストの URL を基準に代理する。継続 MPEG-TS はストリーム転送し、上流認証情報をブラウザへ公開しない。

検証は統一 API、安定 ID、時間範囲、混合ピン留め、無効・空・認証失敗・タイムアウト・復旧、
入れ子 HLS、MPEG-TS、連続切り替え、退出、古い応答、セッション回収を回帰対象とする。
フロントエンドの lint / typecheck と関連テスト、バックエンドの lint と関連テストを実行し、旧経路の残存を確認する。
実環境の HTTP とブラウザではホーム・番組表からの視聴、切り替え、更新、戻る、障害復旧、デスクトップと狭幅の配置・フォーカスを確認する。
既存放送の再生を維持する。ユーザー管理のバックエンドを無断で起動・再起動しない。
実サービスで確認できない項目は未検証と明記し、ビルドや模擬応答を実再生の証拠としない。
...
- **リモート進捗バーと CM スキップ操作**: 録画再生中のテレビの再生位置・長さ・CM 区間を Web UI の進捗バーへ表示し、ドラッグによる任意位置へのシーク、チャプター送り／戻し、CM 区間から本編への復帰、CM スキップ設定（オフ／手動／自動）の切り替えを操作可能
...
Jellyfin Live TV をネットテレビとして利用する場合は、Jellyfin の接続情報を `general.jellyfin` に設定してください。
`enabled` を `true` にし、`url`、`username`、`password` を設定すると、Jellyfin のチャンネルと番組表を既存の「ネット」タブで利用できます。これらの認証情報はサーバー内だけで使用し、サーバー設定 API やブラウザへ返しません。
